...
void SpikeEffect::RenderFrame() {
  ...
  for (int i = 0; i < 15; ++i) {
    for (int j = 0; j < 15; ++j) {
      ...
      for (int k = 0; k < 8; ++k) {
        ...
        if (id > 0 && id < num_pixels_ && i + columnoffset[k] >= 0 &&
            i + columnoffset[k] < 15 && j + rowoffset[k] >= 0 &&
            j + rowoffset[k] < 15 && pixels_[id].w != 0) {
  ...
  StarsEffect::StarsEffect(buffer_pixel_t * pixels, int num_pixels,
                           int num_stars)
